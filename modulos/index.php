        ...
        for ($row = 0; $row < count($csv); $row++) {
            ...
            for ($col = 0; $col < count($csv[0]); $col++) {
                if ($col == 0) {
                    ...
                    $csv[$row][$col] = $control[$rowName][$col-1];
                }
            }
        }
...
function printControlTable()
{
    ...
    if ( ! file_exists($controlFilePath)) {
        ...
        for ($rowIndex = 0; $rowIndex < count($csv); $rowIndex++) {
            ...
            for (
                $columnIndex = 0; $columnIndex < count($csv[0]); $columnIndex++
            ) {
                echo '<'.$cellTag.'>';
                if ($rowIndex === 0 || $columnIndex === 0) {
                    ...
                    $controlValue = $csv[$rowIndex][$columnIndex];
                    echo '<div class="form-check form-switch">
                      <select class="form-select form-select-sm"
                       name="control['.$rowName.'][]">
                        <option value="1" '.($controlValue == "1" ? 'selected'
                            : '').'>Activo</option>
                        <option value="0" '.($controlValue == "0" ? 'selected'
